...
// ─── Auth — Admin (platform JWT + is_admin = TRUE) ───────────────────────────
//
// Requiere autenticación de plataforma Y que el usuario tenga is_admin = TRUE.
// El chequeo es server-side contra DB — nunca confiar en el JWT solo.
// Loggea cada acceso en public.admin_access_log.
...
// Crear tabla al arrancar (una sola vez, idempotente)
ensureAdminLogTable().catch(() => {});

async function requireAdmin(req, reply) {
  // 1. Verificar JWT de plataforma
  await requirePlatformAuth(req, reply);
  if (reply.sent) return;

  // 2. Token reciente — para rutas admin el JWT no puede tener más de 2h
  //    (el access token dura 1d, pero admin requiere sesión reciente)
  const payload = req.server.jwt.decode(req.headers.authorization.slice(7));
  const iat = payload?.iat || 0;
  if (Date.now() / 1000 - iat > 2 * 60 * 60) {
    return reply.code(401).send({ error: "Session too old — re-login required for admin access" });
  }

  // 3. Verificar is_admin en DB (revocable sin tocar el JWT)
  ...
  if (!rows[0]?.is_admin) {
    // Log intento fallido
    db.query(
      `INSERT INTO public.admin_access_log (user_id, method, path, ip, user_agent)
       VALUES ($1, $2, $3, $4, $5)`,
      [req.user.id, req.method, req.url, req.ip, req.headers["user-agent"] || null]
    ).catch(() => {});
    return reply.code(403).send({ error: "Forbidden" });
  }

  // 3. Log acceso exitoso
  db.query(
    `INSERT INTO public.admin_access_log (user_id, method, path, ip, user_agent)
     VALUES ($1, $2, $3, $4, $5)`,
    [req.user.id, req.method, req.url, req.ip, req.headers["user-agent"] || null]
...
async function requireProjectOrPlatformAuth(req, reply) {
  try {
    // Accept service key (x-matecito-key) as an alternative to Bearer JWT
    ...
    if (rawKey) {
      ...
      const ctx = await getProjectKeyContext(projectId, rawKey, ["service"]);
...
async function isProjectMember(userId, projectId) {
  const result = await db.query(
    `SELECT p.id, p.workspace_id, p.schema_name, wm.role
     FROM projects p
     JOIN workspace_members wm ON wm.workspace_id = p.workspace_id
     WHERE p.id = $1 AND wm.user_id = $2
     LIMIT 1`,
...
async function createProjectSchema(executor, schemaName) {
  ...
  await executor.query(`
    ...
    END $$
  `);
  await executor.query(`GRANT USAGE, CREATE ON SCHEMA "${schemaName}" TO "${schemaName}"`);
