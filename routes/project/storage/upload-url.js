...
module.exports = async function (fastify) {
  const handler = async (req, reply) => {
    ...
    try {
      ...
      return reply.code(400).send({ error: `Failed to fetch URL: ${err.message}` });
    }

    if (!fetchRes.ok) {
      return reply.code(400).send({ error: `Remote URL returned status ${fetchRes.status}` });
    }
    ...
    const publicUrl = `${STORAGE_URL}/${projectId}/${outFilename}`;
