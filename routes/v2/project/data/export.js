...
function toCSV(rows) {
  ...
  for (const row of rows) {
    if (row.data && typeof row.data === "object") Object.keys(row.data).forEach(k => dataKeys.add(k));
  }
...
module.exports = async function (fastify) {
  const handler = async (req, reply) => {
    ...
    const expiredFilter = include_expired === "true" ? "" : "AND (expires_at IS NULL OR expires_at > NOW())";

    const { rows } = await db.query(
      `SELECT * FROM ${schema}._records WHERE collection = $1 ${expiredFilter} ORDER BY created_at ASC LIMIT $2`,
